...
router.post('/uploadImage', upload.single('image'),processImage, (req, res) => {
  ...
  console.log(processedImageFileName);
  // Return a response containing the image details
  res.status(200).json({ imageURL: processedImageFileName }); // You can customize the response structure
});
...
router.put('/updateStores/:storesId', upload.single('logo'), async (req, res) => {
  ...
  console.log("phoneNumber55",phoneNumber)
  try {
...
router.post('/addCompany', upload.fields([{ name: 'image', maxCount: 1 }, { name: 'banner', maxCount: 1 }]), async (req, res) => {
  ...
  try {
    ...
    // console.log("Response from Uber API:", response);
    const responseData = await response.json();
    // console.log("Response data:", responseData);
    
    // Vérifier si la création de l'organisation Uber a réussi
...
console.log(name)
    ...
    // Vérifier si le tag existe déjà pour ce magasin
   /* const existingTag = await Tags.findOne({ name, storeId });
    if (existingTag) {
      return res.status(400).json({ message: 'Tag already exists for this store' });
    }*/
...
console.log(tags)
// Retourner les tags en tant que réponse JSON
